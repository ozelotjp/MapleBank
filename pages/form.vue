    ...
    v-container
      v-row
        v-col(cols="12")
          ...
          v-btn.mr-1.mb-1(v-for="(item, id) in payment" :key="id" @click="select(item.text, item.price), dialog = true") {{ item.text }}
        v-col(cols="12")
          v-alert(dark dense) 入金ボタン
          v-btn.mr-1.mb-1(@click="select('クロワッサン', -183), dialog = true") クロワッサン
          v-btn.mr-1.mb-1(v-for="(item, id) in receipt" :key="id" @click="select('入金', item), dialog = true") {{ item }}円
        v-col(cols="12")
